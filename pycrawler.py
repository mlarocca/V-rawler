...
'''
  Created on 28/nov/2012
   ...
   PyCrawler is a Breadth-first crawler, keeping a queue of 
   ...
   The actual crawling is made by class Crawler.
...
#Important!!!
urlfetch.set_default_fetch_deadline(8)  #Wait max 8 seconds per page (default was 5)
...
#    logging.info("Crawling page: %s; time:%s\n" % (url, datetime.now())) #Logs current page in order to give signals of its activity
    ...
    if page.status_code != 200:
      logging.error("Error: can't open %s - status code %d" % (url, page.status_code))
      return None   
  ...
  def startParsing(self, url):
    ...
    if html != None and self.__handler.check_page_by_content(html, url):
      try:
        self.__page._page_size = len(html)
        self.feed(html)
...
class Crawler():
  ...
  def __crawl_next(self):
    ...
    try:
      page_url = self.__handler._queue.get(False)
    except Empty:
      return
...
#        scheme = 'http'
  ...
  def start_crawling(self, url, max_page_depth = None, max_pages_to_crawl = None, crawler_delay = DEFAULT_CRAWLER_DELAY):
...
#      home_scheme = 'http'
  ...
  def page_graph(self, page_url = None):
    ...
    _, home_domain, home_path, _, _ = urlsplit(page_url)
    ...
    def transform_video_urls(video, page_url):
      ...
      video[VIDEO_URLS_TAG] = list(map(lambda url: urljoin(page_url, url), video[VIDEO_URLS_TAG]))
    ...
    def process_page(page, page_url):
      ...
      _, domain, path, _, _ = urlsplit(page_url)
      if domain != home_domain:
        path = domain + path
      
      return  { "page_size": page._page_size,
                "depth": page._depth,
                "path": path,
                "links": list(page._links),
                "resources":  {"images": list(map(lambda url: urljoin(page_url, url), page._img_urls)), 
                               "videos": [transform_video_urls(video, page_url) for video in page._videos if len(video[VIDEO_URLS_TAG]) > 0], 
                               "audios": [list(map(lambda url: urljoin(page_url, url), audio)) for audio in page._audios if len(audio) > 0]
                              }
              } 
